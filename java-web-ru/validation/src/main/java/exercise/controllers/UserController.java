...
Validator<String> lastNameValidator = ctx.formParamAsClass("lastName", String.class)
        .check(it -> !it.isEmpty(), "Фамилиля не должна быть пустой");
...
Validator<String> passwordValidator = ctx.formParamAsClass("password", String.class)
        .check(it -> StringUtils.isNumeric(it), "Должны быть только цифры")
        .check(it -> it.length() >= 4, "Пароль должен быть длиннее 4 цифр");
